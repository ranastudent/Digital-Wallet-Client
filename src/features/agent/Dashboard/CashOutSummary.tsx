...
import { useGetAgentCashOutSummaryQuery } from "@/redux/features/transactions/transactionApiSlice";

export default function CashoutSummary() {
  const { data, isLoading, isError } = useGetAgentCashOutSummaryQuery();
      ...
      <div className="hidden md:block overflow-x-auto rounded-md border bg-background">
        <table className="min-w-[600px] w-full table-auto border-collapse">
          <thead className="bg-muted/50">
            <tr>
              <th className="h-9 py-2 px-2 text-left">From (User)</th>
              <th className="h-9 py-2 px-2 text-left">To (Agent)</th>
